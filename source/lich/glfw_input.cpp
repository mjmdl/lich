...
static int
to_glfw_key_code_(Key_Code code)
{
	return static_cast<int>(code);
}

static int
to_glfw_mouse_code_(Mouse_Code code)
{
	return static_cast<int>(code);
}

static Key_Action
from_glfw_key_action_(int action)
{
	switch (action)
	{
...
bool Glfw_Input::
_mouse_down(Mouse_Code code)
{
	Key_Action action = _mouse(code);
	return action == Key_Action::Down or action == Key_Action::Repeat;
}

std::pair<F64, F64> Glfw_Input::
_mouse_pos()
{
	F64 xpos;
	F64 ypos;
