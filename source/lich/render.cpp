...
void Renderer::
submit(const std::unique_ptr<Vertex_Array> &vertex_array)
{
	vertex_array->bind();
	Render_Command::draw_indexed(vertex_array);
}

Orthographic_Camera_2d::
Orthographic_Camera_2d(float left, float right, float bottom, float top)
	: _projection{glm::ortho(left, right, bottom, top, -1.0f, 1.0f)}, _view{1.0f}, _left{left}, _right{right}, _bottom{bottom}, _top{top}
{
	_view_projection = _projection * _view;
}

const glm::vec3 &Orthographic_Camera_2d::
position() const
{
	return _position;
}

float Orthographic_Camera_2d::
rotation() const
{
	return _rotation;
}

const glm::mat4 &Orthographic_Camera_2d::
projection() const
{
	return _projection;
}
...
_recalculate_view_matrix()
{
	...
	glm::mat4 transform = glm::translate(identity, _position) * glm::rotate(identity, _rotation, up);
...
tl::expected<std::unique_ptr<Vertex_Array>, std::string> Vertex_Array::
create()
{
