...
void
check_opengl_error(const char *file, int line)
{
...
void
assert_opengl_error(const char *file, int line)
{
	GLenum error = glGetError();
	if (error != GL_NO_ERROR)
	{
...
Usize
component_count_of(Shader_Data_Type type)
{
	switch (type)
	{
...
static GLenum
equivalent_opengl_type(Shader_Data_Type type)
{
	switch (type)
	{
...
void Opengl_Vertex_Array::
add_vertex_buffer(std::unique_ptr<Vertex_Buffer> &&vbo)
{
	GL_CHECK(glBindVertexArray(_vao));
	vbo->bind();
	...
	for (const auto &attrib : vbo->layout().attribs)
	{
		...
		GL_CHECK(glVertexAttribPointer(location, components, type, normalized, stride, (void *)offset));
	...
	if (_vertex_count != 0 and vertex_count != _vertex_count)
		log_warn("Adding a vertex buffer of different number of elements: From {} to {}.", _vertex_count, vertex_count);
...
void Opengl_Vertex_Array::
set_index_buffer(std::unique_ptr<Index_Buffer> &&ebo)
{
	GL_CHECK(glBindVertexArray(_vao));
	ebo->bind();
...
const std::unique_ptr<Index_Buffer> &Opengl_Vertex_Array::
index_buffer() const
{
	return _index_buffer;
}

Usize Opengl_Vertex_Array::
vertex_count() const
{
	...
	: _layout{}, _count{count}
{
	...
	GL_CHECK(glBufferData(GL_ARRAY_BUFFER, count * (sizeof *vertices), vertices, GL_STATIC_DRAW));
}
...
const Buffer_Layout &Opengl_Vertex_Buffer::
layout() const
{
	return _layout;
}

Usize Opengl_Vertex_Buffer::
size() const
{
	...
	: _count{count}
{
	...
	GL_CHECK(glBufferData(GL_ELEMENT_ARRAY_BUFFER, count * (sizeof *indices), indices, GL_STATIC_DRAW));
}
...
compile(const std::string &vertex_source, const std::string &fragment_source)
{
	...
	if (!vertex)
	{
		return tl::unexpected{fmt::v11::format("Failed to compile a GLSL vertex shader: {}", vertex.error())};
	}
	...
	if (!fragment)
	{
		...
		return tl::unexpected{fmt::v11::format("Failed to compile a GLSL fragment shader: {}", fragment.error())};
	}
	...
	if (status == GL_FALSE)
	{
		...
		return tl::unexpected{fmt::v11::format("Failed to link a GLSL program: {}", log)};
	}
...
upload_uniform(const std::string &name, const glm::mat4 &matrix)
{
	...
	if (location < 0)
		log_warn("GLSL uniform location '{}' not found.", name);
