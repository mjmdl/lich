...
static Key_Code
to_our_key_code_(int glfw_key_code)
{
	return static_cast<Key_Code>(glfw_key_code);
}

static Mouse_Code
to_our_mouse_code_(int glfw_mouse_code)
{
	return static_cast<Mouse_Code>(glfw_mouse_code);
}

std::unique_ptr<Window> Window::
create(const Window_Spec &window_spec)
{
	return std::make_unique<Glfw_Window>(window_spec);
}

void Glfw_Window::
glfw_error_callback_(int error, const char *description)
{
	...
	: _window{NULL}, _title{window_spec.title}, _event_callback{}, _success{false}
{
	...
	_window = glfwCreateWindow(window_spec.width, window_spec.height, _title.c_str(), NULL, NULL);
	if (_window == NULL)
	{
...
window_self_(GLFWwindow *handle)
{
...
void Glfw_Window::
set_event_callback(const Event_Callback &callback)
{
...
void Glfw_Window::
glfw_close_callback_(GLFWwindow *window)
{
...
void Glfw_Window::
glfw_focus_callback_(GLFWwindow *window, int focused)
{
	auto self = window_self_(window);
...
void Glfw_Window::
glfw_pos_callback_(GLFWwindow *window, int xpos, int ypos)
{
	auto self = window_self_(window);
...
void Glfw_Window::
glfw_size_callback_(GLFWwindow *window, int width, int height)
{
...
void Glfw_Window::
glfw_key_callback_(GLFWwindow *window, int key, [[maybe_unused]] int scancode, int action, [[maybe_unused]] int mods)
{
	auto self = window_self_(window);
...
void Glfw_Window::
glfw_mouse_button_callback_(GLFWwindow *window, int button, int action, [[maybe_unused]] int mods)
{
	auto self = window_self_(window);
	if (action == GLFW_RELEASE)
	{
